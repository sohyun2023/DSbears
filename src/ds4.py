...
xgb_model.load_model('models/0819_xgboost_model.json') 
...
def preprocess_data_fixed(data):
    ...
    data.drop(['week_x', 'day_num', 'date'], axis=1, inplace=True)
    ...
    # 피처를 학습된 모델의 피처 순서에 맞춰 재정렬
    data = data[model_feature_names]
...
# XGBoost 모델 로드
xgb_model = xgb.XGBRegressor()
xgb_model.load_model('models/0819_xgboost_model.json')  # 모델 경로 수정 필요
...
if st.button('GO⚾️'):
    ...
    if stadium_name in stadium_capacity:
        actual_attendance = prediction[0] * stadium_capacity[stadium_name]
        ...
        if actual_attendance < 10000:
            ...
            lower_bound = actual_attendance - 500
            upper_bound = actual_attendance + 900
            
        result_text = f'{stadium_name}의 예상 관중 수:<br>{lower_bound:.0f}~{upper_bound:.0f}'
    else:
